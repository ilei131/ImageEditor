...
fn generate_icns(path: &str, output: &str) -> Result<bool, String> {
    ...
    if width < 1024 || height < 1024 {
        return Err(format!(
            "Image size too small. Required at least 1024x1024, but got {}x{}",
            width, height
        ));
    }
    ...
    for size in sizes {
        ...
        let icns_image =
            icns::Image::from_data(PixelFormat::RGBA, size, size, pixels).map_err(|e| {
                format!(
                    "Failed to create ICNS image for size {}x{}: {:?}",
                    size, size, e
                )
            })?;
...
fn get_mouse_position_and_color() -> Result<HashMap<String, serde_json::Value>, String> {
    #[cfg(target_os = "windows")]
    {
        ...
        unsafe {
            ...
            result.insert(
                "color".to_string(),
                serde_json::Value::String(hex_color.clone()),
            );
            result.insert(
                "note".to_string(),
                serde_json::Value::String("Windows 鼠标拾色".to_string()),
            );
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        match color_result {
            Ok(hex_color) => {
                ...
                result.insert(
                    ...
                    serde_json::Value::String(
                        "macOS 系统拾色器已启动，请在 Colors 应用中选择颜色".to_string(),
                    ),
                );
            ...
            Err(e) => {
                ...
                result.insert(
                    "color".to_string(),
                    serde_json::Value::String("#808080".to_string()),
                );
                result.insert(
                    "note".to_string(),
                    serde_json::Value::String(format!("无法启动系统拾色器: {}", e)),
                );
    ...
    #[cfg(not(any(target_os = "windows", target_os = "macos")))]
    {
        ...
        result.insert(
            "note".to_string(),
            serde_json::Value::String("Color picker not supported on this platform".to_string()),
        );
...
fn start_macos_color_picker() -> Result<String, String> {
    ...
    tell process "Color Picker"
        ...
        let _ = Command::new("osascript").arg("-e").arg(rgb_script).output();
...
fn get_mouse_position_macos() -> i32 {
    ...
    match Command::new("osascript")
        .args(&[
            "-e",
            "tell application \"System Events\" to get x of position of mouse",
        ])
        .output()
    {
        Ok(output) => {
...
fn get_mouse_position_macos_y() -> i32 {
    ...
    match Command::new("osascript")
        .args(&[
            "-e",
            "tell application \"System Events\" to get y of position of mouse",
        ])
        .output()
    {
        Ok(output) => {
...
fn extract_colors(path: &str) -> Result<Vec<ColorItem>, String> {
    ...
    let mut total_valid_pixels: u32 = 0;
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let g = channels[1];
            let b = channels[2];
            let a = if channels.len() > 3 { channels[3] } else { 255 };

            // 跳过透明像素（alpha值低于128的视为透明）
            if a < 128 {
                continue;
            }
    ...
    // 合并相似的颜色（阈值降低到500，减少过度合并）
    let color_vec: Vec<(String, u32)> = color_counts.into_iter().collect();
    let merged_colors = merge_similar_colors(color_vec, 500.0);
    ...
    // 计算每种颜色的占比，并过滤掉占比低于0.2%的颜色
    let min_percentage = 0.2; // 最小占比阈值
    let mut colors: Vec<ColorItem> = merged_color_counts
        .into_iter()
        .filter_map(|(hex, count)| {
            let percentage = (count as f32 / total_valid_pixels as f32) * 100.0;
            // 只保留占比大于等于0.2%的颜色
            if percentage >= min_percentage {
                Some(ColorItem { hex, percentage })
            } else {
                None
            }
        })
